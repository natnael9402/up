...
import { getMiningHostings, terminateMiningHosting } from '@/lib/api';
...
export default function MiningHostingsPage() {
  const [items, setItems] = useState<MiningHosting[]>([]);
  const [loading, setLoading] = useState(true);
  const [processing, setProcessing] = useState<number | null>(null);
  const [terminateTarget, setTerminateTarget] = useState<MiningHosting | null>(null);
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await getMiningHostings();
      setItems(data);
    } finally {
      setLoading(false);
    }
  ...
  async function onTerminate(id: number) {
    ...
    try {
      await terminateMiningHosting(id);
      notify('success', 'Mining hosting terminated successfully');
      await load();
    } catch {
      notify('error', 'Failed to terminate mining hosting');
    } finally {
      setProcessing(null);
    }
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <ConfirmModal
        ...
        title="Terminate Mining Hosting"
        message={`Are you sure you want to terminate this ${terminateTarget?.planCode ?? ''} mining hosting? This will end the plan and process any pending profits.`}
      ...
      <main className="container mx-auto p-4 lg:p-8 lg:pt-16">
        <PageHeader title="Mining Hostings" subtitle="Manage active mining hosting plans." />
